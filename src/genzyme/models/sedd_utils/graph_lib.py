...
def get_graph(config, device):
    if config.graph.type == "uniform":
        return Uniform(config.tokens, config.training.loss == "dwdse")
    elif config.graph.type == "absorb":
        return Absorbing(config.tokens, config.training.loss == "dwdse")
    else:
...
class Graph(abc.ABC):
    ...
    def reverse_rate(self, i, score):
        ...
        """
        normalized_rate = self.transp_rate(i).to(score) * score # -delta(x_t,j) + delta(x_t, M)

        # normalize rate matrix row sums (change rate for x_t=j)
    ...
    def sample_rate(self, i, rate, last=False):
        logits = F.one_hot(i, num_classes=self.dim).to(rate) + rate
        if last and self.absorb:
            logits = logits[...,:-1]
        return sample_categorical(logits) #delta(j, x_t) + rate
...
class Uniform(Graph):
    ...
    """
    def __init__(self, dim, exp=True):
        self._dim = dim
        self._exp = exp
    ...
    def transp_rate(self, i):
        if self._exp:
            ...
            return torch.ones(i.size()+(self.dim,)) # all ones matrix
    ...
    def sample_transition(self, i, sigma):
        move_chance = (1 - (-sigma).exp()) if self._exp else sigma
    ...
    def cross_entropy(self, logits, x0):
        return F.cross_entropy(logits.transpose(1,2), x0, reduction="mean")
...
class Absorbing(Graph):
    ...
    def rate(self, i):
        ...
        # return 1 for absorb token if i does not have absorb in that position
        return F.one_hot((self.dim - 1) * torch.ones_like(i), num_classes=self.dim) - F.one_hot(i, num_classes=self.dim)        

    def transp_rate(self, i):
        if self._exp:
            edge = -F.one_hot(i, num_classes=self.dim) # -delta(x_t,j) + delta(x_t, M)
            edge[i == self.dim - 1] += 1 # set to 0 if x_t is absorb token
        else:
            edge = torch.zeros(i.size()+(self.dim,))
            edge[i == self.dim-1] = 1 # delta(x_t, M)
    ...
    def cross_entropy(self, logits, x, x0):
        x0_msk = x0.clone()
        x0_msk[x != self.dim-1] = -1 # only compute loss on masked tokens
        return F.cross_entropy(logits.transpose(1,2), x0_msk, ignore_index = -1, reduction = "mean")
