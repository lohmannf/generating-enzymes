...
def get_loss_fn(noise, graph, train, sampling_eps=1e-3, lv=False, ltype = "dwdse"):
    ...
    def loss_fn(model, batch, cond=None, t=None, perturbed_batch=None):
        ...
        sigma, dsigma = noise(t)   #total noise, rate noise !!
        ...
        if ltype == "dwdse":
            ...
            loss = graph.cross_entropy(log_score, perturbed_batch, batch)

        else:
            raise NotImplementedError(f"Unknown loss type {ltype}")
...
def get_step_fn(noise, graph, train, optimize_fn, accum, ltype="dwdse"):
    loss_fn = get_loss_fn(noise, graph, train, ltype=ltype)
